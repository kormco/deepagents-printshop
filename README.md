...
### Research Report
A professional academic-style research report with tables, figures, and citations.

**Features:**
- Framed disclaimer notice at the start
- Auto-generated table of contents
- Data tables from CSV files
- Performance comparison charts
- "Generated by DeepAgents PrintShop" attribution

📄 **Output:** `artifacts/output/research_report.pdf` (~300KB, 6-7 pages)
...
📄 **Output:** `artifacts/output/magazine.pdf` (~7MB, 10-15 pages)
...
3. Generate a `config.md` with appropriate metadata:
   ```markdown
   # Document Configuration

   ## DISCLAIMER
   **THIS IS A DRAFT DOCUMENT FOR REVIEW PURPOSES.**
   [Your custom disclaimer here]

   ## Title
   Machine Learning Model Evaluation: A Comparative Study
...
│   ├── research_report.pdf       # Research report PDF (~300KB)
│   ├── research_report.tex       # Research report LaTeX source
│   ├── magazine.pdf              # Magazine PDF (~7MB with images)
│   └── magazine.tex              # Magazine LaTeX source
├── reviewed_content/              # Versioned outputs (created at runtime)
│   ├── v2_latex_optimized/
...
│       └── iterations/
│           └── iteration_*.pdf   # Progressive improvements
├── agent_reports/
│   ├── quality/
...
2. Add a `config.md` with document settings and disclaimer
3. Place markdown files in the folder
4. Add CSV tables to `artifacts/sample_content/my_document/data/`
5. Add images to `artifacts/sample_content/my_document/images/`
6. Run the pipeline with: `python agents/qa_orchestrator/agent.py --content my_document`
