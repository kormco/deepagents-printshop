...
class LaTeXOptimizer:
    ...
    def _fix_common_llm_typos(self, tex_content: str) -> str:
        """Fix common LaTeX command typos generated by LLMs.

        LLMs sometimes generate typos in LaTeX commands. This function
        corrects known common typos.
        """
        typo_fixes = {
            # Bibliography typos
            r'\\biboc\b': r'\\bibitem',
            r'\\bibitm\b': r'\\bibitem',
            r'\\bibtem\b': r'\\bibitem',
            r'\\bitem\b': r'\\bibitem',
            # Section typos
            r'\\secton\b': r'\\section',
            r'\\subsecton\b': r'\\subsection',
        ...
        for typo_pattern, correct in typo_fixes.items():
            tex_content = re.sub(typo_pattern, correct, tex_content)

        return tex_content

    def _close_unclosed_environments(self, tex_content: str) -> str:
        """Ensure all LaTeX environments are properly closed.

        LLMs sometimes forget to close environments like thebibliography.
        This function detects unclosed environments and appends the missing \\end{}.
        """
        # Environments that commonly get left unclosed by LLMs
        ...
        for env in environments_to_check:
            ...
            opens = len(re.findall(rf'\\begin\{{{env}}}', tex_content))
            closes = len(re.findall(rf'\\end\{{{env}}}', tex_content))

            # Add missing closes at the end
            if opens > closes:
